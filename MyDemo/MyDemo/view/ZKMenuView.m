...
@interface ZKMenuView()
@property (nonatomic, assign) CGPoint origin;
@property (nonatomic, assign) CGFloat rowHeight;
@property (nonatomic, assign) CGFloat width;
@property (nonatomic, copy) NSString *key;
@end
@implementation ZKMenuView

- (instancetype)initWithDataSource:(NSArray *)dataSource Origin:(CGPoint)origin Width:(CGFloat)width RowHeight:(CGFloat)rowHeight KeyString:(NSString *)key
{
    ...
    if (self) {
        ...
        self.width = width;
        self.arrData = [dataSource copy];
        
        CGFloat height = dataSource.count > 5 ? 5 * rowHeight *1.0 : rowHeight*dataSource.count;
...
//        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x + LeftToView, origin.y + TopToView, width, height) style:UITableViewStylePlain];
        ...
        self.tableView = [[UITableView alloc] initWithFrame:CGRectMake(origin.x , origin.y + 10 , width, height) style:UITableViewStylePlain];
        ...
        [_tableView registerClass:[UITableViewCell class] forCellReuseIdentifier:@"ZKMenuView"];
...
{
    UITableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:@"ZKMenuView"];
    if (!cell) {
        cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ZKMenuView"];
    }
...
{
    if([self.delegate respondsToSelector:@selector(ZKCustomMenu:didSelectRowAtIndexPath:)]){
        [self.delegate ZKCustomMenu:tableView didSelectRowAtIndexPath:indexPath];
    }
